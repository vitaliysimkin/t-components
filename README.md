...
Компоненти малюють іконки через `@iconify/vue`. Іконковий набір розділено на два рівні:

**Core (за замовчанням, ~kB).** Просто імпортуй компоненти — вони працюють із коробки:

```ts
import { TButton } from '@vitaliysimkin/t-components'
```

Curated-набір `ticon:*` (дефолтні іконки компонентів + fallback `ticon:missing`) реєструється автоматично. Жодного мережевого запиту, нічого додатково підключати не треба.

**Optional offline `system-uicons` (opt-in).** Якщо хочеш малювати будь-які `system-uicons:*` імена офлайн (без запиту до `api.iconify.design`), підключи важкий набір один раз на старті застосунку:

```ts
// main.ts — один раз
import '@vitaliysimkin/t-components/icons'
```

Цей імпорт реєструє весь `system-uicons` набір (і включає `ticon:*`, тож самодостатній). Core-бандл його НЕ тягне — платиш за нього лише якщо явно імпортуєш.

**Невідомі імена.** Якщо передати неіснуюче/незареєстроване ім'я іконки, `<TIcon>` (і всі компоненти на його основі) намалює помітний `ticon:missing` гліф з підказкою (`title`), а не порожнечу — щоб одруківки й незареєстровані імена одразу впадали в око.
