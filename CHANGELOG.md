...
- Бібліотека тепер бандлить увесь набір `system-uicons` офлайн (`addCollection`) — споживачам більше не треба реєструвати його вручну й не потрібні мережеві запити до api.iconify.design.
- Кураторський офлайн-набір іконок під префіксом `ticon:*` (12 гліфів Material Symbols + `ticon:missing`), що використовуються як дефолти компонентів.
