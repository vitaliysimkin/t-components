...
function initialDateForView(): Date {
  ...
  if (props.mode === 'range' && v.start instanceof Date) return v.start;
...
watch(
  ...
  (newValue) => {
    ...
    if (props.mode === 'range' && newValue.start instanceof Date) {
      d = newValue.start;
...
function isDateSelected(date: Date): boolean {
  ...
  if (props.mode === 'range') {
    if (!v.start || !v.end) {
      return v.start ? isSameDay(v.start, date) : false;
    }
    return isSameDay(v.start, date) || isSameDay(v.end, date);
  }
  if (v instanceof Date) return isSameDay(v, date);
  return false;
}
...
function isInRange(date: Date): boolean {
  ...
  return t > startOfDay(v.start) && t < startOfDay(v.end);
}
...
function isRangeEdge(date: Date, edge: 'start' | 'end'): boolean {
  ...
  return isSameDay(v[edge], date);
}

function isMonthSelected(mIdx: number, y: number): boolean {
  const v = props.modelValue;
  if (!v) return false;
  const d = props.mode === 'range' ? v.start : v;
...
function onDayClick(date: Date) {
  ...
  const v = props.modelValue;
  const hasCompleteRange = v && v.start instanceof Date && v.end instanceof Date;
