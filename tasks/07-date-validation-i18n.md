...
- [ ] У `TDateInput.parseDate` після `new Date(y, mo-1, d)` — перевірка `d.getFullYear() === y && d.getMonth() === mo-1 && d.getDate() === d`. Якщо ні — return null (= revert).
- [ ] Аналогічно в `TDateTimeInput.commitText` ([:125-141](../src/components/TDateTimeInput.vue#L125-L141)).
...
- [ ] `TDateInput` / `TTimeInput` / `TDateTimeInput` — додано опціональний `placeholder?: string` з поточним дефолтом (українською для backward-compat).
- [ ] `TDateTimeInput` — додано `doneLabel?: string` з дефолтом `'Готово'`.
- [ ] `TInputModalBox` ([TInputModalBox.vue:71-72](../src/components/modal/TInputModalBox.vue#L71-L72)) — додано `submitLabel?: string` (default `'Submit'`), `cancelLabel?: string` (default `'Cancel'`). Примітка: це також дає i18n-стратегію через `modalManager.openInputModal({ submitLabel: t('save'), ... })`.
