...
- [ ] **Installation** — `npm i @vitaliysimkin/t-components` + peer-deps hint.
- [ ] **Usage: global plugin** — приклад з `app.use(TComponents)`.
- [ ] **Usage: tree-shaking** — приклад з named imports, пояснити trade-off.
- [ ] **CSS import** — `import '@vitaliysimkin/t-components/style.css'` в точці входу.
- [ ] **Theme** — приклад з `useTheme()` + пояснення `html.dark`/`html.light` класів. Короткий FOUC-hint: inline script у `<head>` для раннього застосування теми (див. [../review.md — можливості #8](../review.md#8-dark-mode-без-fouc)).
- [ ] **TypeScript / Volar** — існуюча секція. Залишити як є.
- [ ] **Peer dependencies** — чому `@iconify/vue` і `@vueuse/core` required, чому `vue-router` optional.
- [ ] **SSR caveats** — наразі `useTheme`/`useNotifications`/`useModalManager` мають module-level state і читають `window`/`localStorage` під guard'ами. Явно: "повна SSR-підтримка наразі не гарантується".
- [ ] **Live demo link** — вже є, лишити.
- [ ] **Публікація нової версії** — існуюча секція, залишити.

### CHANGELOG.md (new)

- [ ] Створено `CHANGELOG.md` у форматі [Keep a Changelog](https://keepachangelog.com/en/1.1.0/).
- [ ] Реконструювати зміни з git-історії (останні 7 релізів: 0.1.0 → 0.7.0). Використовуй `git log --oneline` для рушія.
...
- [ ] Unreleased секція зверху — порожня-готова для наступного PR.
